...
/// @param pos  The start of the vector where data should go.  It is assumed to
...
vector<uint8_t> reliable_get_to_eof(int sd) {
  ...
  vector<uint8_t> res;
  // start reading.
  int bytes_received = buffer.capacity();
  while((bytes_received = recv(sd, buffer.data(), buffer.capacity(), 0))){
    ...
    // bytes received.  copy buffer to vector res
    else {
      res.insert(res.end(), buffer.begin(), buffer.begin() + bytes_received);
      if (bytes_received < (int)buffer.capacity())
        break;
      buffer.clear();
    }
  }
...
bool accept_client(int sd, function<bool(int)> handler) {
  ...
  while (true) {
    ...
    ContextManager cs([&]() { close(connSd); });
    char cliName[1024];
    cout << "Connected to "
