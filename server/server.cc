...
struct arg_t {
  ...
  arg_t(int argc, char **argv) {
    long opt;
    while ((opt = getopt(argc, argv, "p:f:ht:b:i:u:d:r:o:a:")) != -1) {
      switch (opt) {
      case 'p':
        port = atoi(optarg);
        break;
...
int main(int argc, char **argv) {
  ...
  cout << "Listening on port " << args->port << endl;
  ...
  // Start accepting connections and handling them
  accept_client(sd,
                [&](int sd) { return parse_request(sd, storage); });
