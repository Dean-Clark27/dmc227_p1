...
/// authentication table.  The authentication table holds user names and
/// passwords, as well as a single vector of content per user.  The
/// second is a key/value store that can be accessed by all threads.  
/// The third is used for managing quotas.
///
...
public:
  ...
  struct result_t {
    ...
    std::vector<uint8_t> data; // Optional additional content to send
  };
  ...
  /// an error.  Otherwise, save an entry with the username, password, 
  /// and a zero-byte content.
  ///
  ...
  ///
  /// @return A result tuple, as described above.  Note that "no data" is an error
