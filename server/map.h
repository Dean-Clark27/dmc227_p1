...
public:
  ...
  ///         otherwise
  virtual bool do_with(K key, std::function<void(V &)> f) = 0;
  ...
  /// @return true if the key existed and the function was applied, false
  ///         otherwise
  virtual bool do_with_readonly(K key, std::function<void(const V &)> f) = 0;
  ...
  virtual void do_all_readonly(std::function<void(const K, const V &)> f) = 0;
};
