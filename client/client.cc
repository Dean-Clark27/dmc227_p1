...
struct arg_t {
  ...
  arg_t(int argc, char **argv) {
    ...
    long opt;
    while ((opt = getopt(argc, argv, "u:w:s:p:C:1:h")) != -1) {
      switch (opt) {
    ...
    // Validate that the argument to `-C` is valid, and accompanied by `-1` as
    // appropriate
    string args0[] = {REQ_BYE, REQ_SAV, REQ_REG};
    string args1[] = {REQ_SET, REQ_GET, REQ_ALL};
    for (auto a : args0) {
      if (command == a) {
        if (arg1 != "")
          throw 1;
  ...
  static void usage(char *progname) {
    ...
    cout << "  -u [string] The username to use for authentication\n"
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < cmd.size(); ++i)
    if (args->command == cmd[i])
      func[i](sd, args->username, args->userpass, args->arg1);
  delete args;
  return 0;
}
