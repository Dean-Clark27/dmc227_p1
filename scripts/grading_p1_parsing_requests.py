...
allfile = "allfile"
makefiles = ["Makefile", "p1.parsing_requests.mk", "p1.storage.mk"]
...
cse303.leftmsg("Copying parsing_requests files into place")
cse303.copyexefile("obj64/server.p1.parsing_requests.exe", "obj64/server.exe")
cse303.copyexefile("obj64/client.p1.parsing_requests.exe", "obj64/client.exe")
cse303.okmsg()

# The parsing requests test runs a battery of GET and SET
print()
cse303.line()
print(
    "Test: Parsing Requests: Stressing Parsing Requests (be sure to run script with SERVER, CLIENT, and no overrides)"
)
cse303.line()
...
cse303.do_cmd(
    "Setting alice's content (binary).", "OK__", client.setC(alice, bfileb1), server
)
cse303.do_cmd(
    "Checking alice's content.", "OK__", client.getC(alice, alice.name), server
)
...
cse303.do_cmd(
    "Overwriting alice's content (binary).", "OK__", client.setC(alice, bfileb2), server
)
cse303.do_cmd(
    "Checking alice's content.", "OK__", client.getC(alice, alice.name), server
)
