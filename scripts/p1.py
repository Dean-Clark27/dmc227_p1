...
client = cse303.ClientConfig("./obj64/client.exe", "localhost", "9999")
...
cse303.do_cmd("Checking alice's content.", "OK__", client.getC(alice, alice.name), server)
cse303.check_file_result(afile1, alice.name)
cse303.do_cmd("Getting all users to make sure it's just alice.", "OK__", client.getA(alice, allfile), server)
cse303.check_file_list(allfile, [alice.name])
cse303.do_cmd("Instructing server to persist data.", "OK__", client.persist(alice), server)
...
cse303.do_cmd("Checking alice's old content.", "OK__", client.getC(alice, alice.name), server)
cse303.check_file_result(afile1, alice.name)
...
cse303.do_cmd("Registering user bob.", "OK__", client.reg(bob), server)
cse303.do_cmd("Attempting to access alice's data by bob.", "OK__", client.getC(bob, alice.name), server)
cse303.check_file_result(afile1, alice.name)
cse303.do_cmd("Getting bob's nonexistent data.", "ERR_NO_DATA", client.getC(bob, bob.name), server)
cse303.do_cmd("Getting all users to make sure it's alice and bob.", "OK__", client.getA(alice, allfile), server)
...
server.pid = cse303.do_cmd_a("Restarting server:", [
    ...
    "Loaded: " + server.dirfile], server.launchcmd())
cse303.waitfor(2)
cse303.do_cmd("Setting alice's content.", "OK__", client.setC(alice, afile2), server)
cse303.do_cmd("Checking alice's content.", "OK__", client.getC(alice, alice.name), server)
