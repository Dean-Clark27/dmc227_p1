...
class ClientConfig:
    ...
    def __init__(self, exe, server, port):
...
def killall(procname):
    ...
    s = subprocess.Popen(
        ["killall", procname], stderr=subprocess.PIPE, stdout=subprocess.PIPE
    )
    s.stderr.readline()
    s.stdout.readline()
...
def check_file_result(f1, f2):
    ...
    print(
        ("  Comparing" + " " + f1 + " and " + f2 + ".file.dat" + ".").ljust(
            indentation
        ),
        end="",
    )
...
def verify_filesize(filename, expect):
    ...
    s = get_len(filename)
    print(
        ("Checking size of " + filename + " (expect " + str(expect) + ")").ljust(
            indentation
        ),
        end="",
    )
...
def next4(num):
    """Raise the value of num to the next multiple of 4"""
    ret = num
    if ret % 4 != 0:
        ret = ret + (4 - ret % 4)
    return ret
